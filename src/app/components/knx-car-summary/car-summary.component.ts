...
import { Car, CarInsuranceOptions } from '../../models';

@Component({
  selector: 'knx-car-summary',
  template: `
  <div class="knx-car-summary knx-message knx-message--hint knx-message--arrow-top">
    <div class="container knx-container--fullwidth">
      ...
      <div class="row" *ngIf="car.edition">
        ...
        <div class="col-sm-6 knx-car-summary__info">{{ car.edition | uppercase }}</div>
      </div>
      <div class="row" *ngIf="car.year">
        <div class="col-sm-6 knx-car-summary__label">Bouwjaar</div>
        <div class="col-sm-6 knx-car-summary__info">{{ car.year }}</div>
      </div>
      <div class="row" *ngIf="car.current_value">
        <div class="col-sm-6 knx-car-summary__label">Cataloguswaarde</div>
        <div class="col-sm-6 knx-car-summary__info">{{ car.current_value | currency:'EUR':true }}</div>
      </div>
      <div class="row" *ngIf="car.price_consumer_incl_vat">
        <div class="col-sm-6 knx-car-summary__label">Dagwaarde</div>
        <div class="col-sm-6 knx-car-summary__info">{{ car.price_consumer_incl_vat | currency:'EUR':true }}</div>
      </div>
...
export class CarSummaryComponent {
  @Input() car: Car;
  @Input() options: CarInsuranceOptions;
