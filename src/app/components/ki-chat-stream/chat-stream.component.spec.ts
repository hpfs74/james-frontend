...
import { AvatarComponent } from './../ki-avatar/avatar.component';
...
describe('Component: ChatStreamComponent', () => {
  ...
  it('should render the specified number of messages', () => {
    expect(el).not.toBeNull();
    expect(el.querySelectorAll('ki-chat-message').length).toBe(messages.length);
  });

  it('should render a vehicle type message', () => {
    expect(el).not.toBeNull();
    expect(el.querySelectorAll('ki-vehicle-info').length).toBe(1);
  });
});
